 ...
 */
	public static  <
		...
		for( Tuple2<K,V> kvTuple : dicKv.keysValues()){
			K k = kvTuple._1();
			V v =kvTuple._2();
			if(dicVk.isDefinedAt(v)) {
				AVLSet<K> m = (AVLSet<K>)dicVk.valueOf(v);
				m.insert(k);
			}else {
				AVLSet<K> m = new AVLSet<K>();
				m.insert(k);
				dicVk.insert(v, m);
			}
		}
 ...
 */
 	public static  <
 		...
 		Dictionary<V,Set<K>> dicVk = new AVLDictionary<V,Set<K>>();
 		dicVk = inverseDictionary(dicKv);
 		
 		V v = null;
 		for( V m : dicKv.values()){
			if(v == null || v.compareTo(m) < 0) {
				v = m;
			}
 		}
		return dicVk.valueOf(v);
	}
