...
merge h@(Node x sz lh rh) h'@(Node x' sz' lh' rh')
 | x < x'          = Node x' (sz+sz') q (merge lh' h)
 | otherwise       = Node x (sz+sz') p (merge lh h')
   where
     (p,p',p'') = pMayor lh rh h'
     (q,q',q'') = pMayor lh' rh' h

pMayor :: Ord a => Heap a -> Heap a -> Heap a -> (Heap a, Heap a, Heap a)
pMayor x h h'
   | (size x >= size h) && (size x >= size h') = (x,h,h')
   | (size h >= size h') = (h,x,h')
   |  otherwise = (h',x,h)
