...
dijkstraAux g vs rs d = dijkstraAux g (v' : vs) (delete v' rs) (D.insert v' (p', cs') d)
   where
      ...
      caminos = D.keysAndValues d
      -- extender los caminos del diccionario con aristas hasta nodo en rs
      extensiones = [(u, p + w, xs ++ [u]) | (x, (p, xs)) <- caminos, (u,w) <- successors g x, elem u rs]
      --- seleccionar la extensión de menor coste
      (v', p', cs') = minimumBy compareEx extensiones

      compareEx ( _, peso1, _) ( _, peso2, _) = compare peso1 peso2
