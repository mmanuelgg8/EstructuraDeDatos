...
--
-- Alumno: Manuel González González
-- Titulación: Ingeniería Informática A
-------------------------------------------------------------------------------
...
primAux g ts rs st = primAux g (y:ts) (delete y rs) (WE x p y : st)
   where
      -- encontrar las aristas de un vértice de ts a otro de rs
      aristas = [(WE x p y) | (WE x p y) <- edges g, x <- ts, y <- rs] 
      -- de todas las aristas calculadas en el paso anterior encuentra la de menor peso
      (WE x p y) = minimumBy comparaArco aristas
          where 
          comparaArco (WE x w y) (WE x' w' y') = compare w w'
