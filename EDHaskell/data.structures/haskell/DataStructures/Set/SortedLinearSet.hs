...
insert x (Node y s)                                            -- p@(Node y s) hace que p sea (Node y s) y se implemente con mayor comodidad
 | x < y        = Node x (Node y s)                            -- | x < y = Node x p
 | x == y       = Node y s
...
--subSet s s' = fold (\e ac -> isElem e s' && ac) True s
subSet Empty s' = True
subSet (Node x s) s' = isElem x s' && subSet s s'
